<%         	
  ...
  '	response.write "SQL = " & SQLStmt2.CommandText
      ...
      Set SQLStmt2 = Server.CreateObject("ADODB.Command")
  	    Set rs2 = Server.CreateObject ("ADODB.Recordset")
  	    SQLStmt2.CommandText = "exec does_staff_form_exist " & precheck_form_value
  	    SQLStmt2.CommandType = 1
  	    Set SQLStmt2.ActiveConnection = conn
  	    SQLStmt2.CommandTimeout = 45 'Timeout per Command
  	    'response.Write "sql = " & SQLStmt2.CommandText
        ...
        'IF FORM ID IS FOUND IN SYSTEM        
          ...
          ' response.end
	        ...
	        SQLStmt3.CommandText = "exec get_staff_form_info " & precheck_form_value
           ...
           ' checked_form_type = rs3("Form_Type")
    	    ...
    	    checked_form_staff = rs3("Staff_Id")
       ...
       if foundSameFormHashMatch = 1 THEN 'THIS MEANS THE SAME FORM WAS SUBMITTED WITH NO SIGNED CHANGES ON IT, PIF AND ESP COMPS ALLOW UNSIGNED CHANGES SO SAVE FORM CONTENT ONLY
           ...
           '  response.Write "foundSameFormHashMatch"
        '   response.end
            ...
            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	        SQLStmtI.CommandText = "exec update_existing_staff_form_content " & checked_form_id & ",'" & new_xml_Contents & "','" & Session("user_name") & "'"
  	        SQLStmtI.CommandType = 1
  	        Set SQLStmtI.ActiveConnection = conn
  	        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	        rsI.Open SQLStmtI
            ...
            '****************************************************************************
            	 ...
            	 '    response.Write "new form submit"
        '   response.end 
            ...
            do while not wfile.AtEndOfStream 
                ...
                if InStr(singleline,"<form_staff_id>") and foundStaffID = 0 THEN
                                    
                    foundStaffID = 1
                    form_id_tag_start = InStr(singleline,"<form_staff_id>")
                    form_id_tag_end = InStr(singleline,"</form_staff_id>")
                    total_tag_length = form_id_tag_end - form_id_tag_start
                    form_StaffID = Mid(singleline,(form_id_tag_start+15),(total_tag_length-15))
               ...
               if InStr(singleline,"<submit_form>") and foundSubmitForm = 0 THEN
                
                    foundSubmitForm = 1
                    submit_form_tag_start = InStr(singleline,"<submit_form>")
                    submit_form_tag_end = InStr(singleline,"</submit_form>")
                    total_tag_length = submit_form_tag_end - submit_form_tag_start
                    form_SubmitForm = Mid(singleline,(submit_form_tag_start+13),(total_tag_length-13))
                    
                end if

            
              if InStr(singleline,"<agency>") and foundProgramID = 0 THEN
                
                        foundProgramID = 1
                        program_id_tag_start = InStr(singleline,"<agency>")
                        program_id_tag_end = InStr(singleline,"</agency>")
    
                        total_tag_length = program_id_tag_end - program_id_tag_start
                        form_ProgramID = Mid(singleline,(program_id_tag_start+8),(total_tag_length-8))
 ...
 ' response.End   
            ...
            'IF FORM ID IS EMPTY 
                ...
                Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	            Set rsI = Server.CreateObject ("ADODB.Recordset")
              if form_FormType="STAFF_EMR_HELPDESK" or form_FormType="STAFF_TRACK_REPORT" then
  	            SQLStmtI.CommandText = "exec insert_update_staff_forms_master_emrhelpdesk '" & form_FormType & "'," & form_StaffID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Add'," & form_LinkedFormID & "," & form_ProgramID
               else
                  SQLStmtI.CommandText = "exec insert_update_staff_forms_master '" & form_FormType & "'," & form_StaffID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Add'," & form_LinkedFormID
              end if
  	            SQLStmtI.CommandType = 1
  	            Set SQLStmtI.ActiveConnection = conn
  	            SQLStmtI.CommandTimeout = 45 'Timeout per Command
                ...
                Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	            Set rsI = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtI.CommandText = "exec insert_update_staff_signatures_completed_datalock " & form_FormID & ",'" & form_DataLock & "', 'Add'"
  	            SQLStmtI.CommandType = 1
  	            Set SQLStmtI.ActiveConnection = conn
  	            SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	            rsI.Open SQLStmtI
            ...
            ELSE                
                ...
                Set SQLStmt2 = Server.CreateObject("ADODB.Command")
  	            Set rs2 = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmt2.CommandText = "exec does_staff_form_exist " & idToFind 
  	            SQLStmt2.CommandType = 1
  	            Set SQLStmt2.ActiveConnection = conn
  	            SQLStmt2.CommandTimeout = 45 'Timeout per Command
  	            'response.Write "sql = " & SQLStmt2.CommandText
  	            ...
  	            IF rs2("form_id_exists") = "Yes" THEN
                    ...
                    Set SQLStmt3 = Server.CreateObject("ADODB.Command")
  	                Set rs3 = Server.CreateObject ("ADODB.Recordset")
	                SQLStmt3.CommandText = "exec get_staff_form_info " & idToFind
                    SQLStmt3.CommandType = 1
                    Set SQLStmt3.ActiveConnection = conn
                    SQLStmt3.CommandTimeout = 45 'Timeout per Command
                    'response.Write "sql = " & SQLStmt3.CommandText
  	                ...
  	                IF rs3("Status") = "Finalized" THEN
                        ...
                        ELSE
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
                            if form_FormType="STAFF_EMR_HELPDESK" or form_FormType="STAFF_TRACK_REPORT" then
  	                        SQLStmtI.CommandText = "exec insert_update_staff_forms_master_emrhelpdesk '" & form_FormType & "'," & form_StaffID & "," & form_FormID & "," & old_form_FormID & ",'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','" & new_form_child_status & "','Add'," & form_LinkedFormID & "," & form_ProgramID
                           else
                             SQLStmtI.CommandText = "exec insert_update_staff_forms_master '" & form_FormType & "'," & form_StaffID & "," & form_FormID & "," & old_form_FormID & ",'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','" & new_form_child_status & "','Add'," & form_LinkedFormID
                           end if
  	                        SQLStmtI.CommandType = 1
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                        SQLStmtI.CommandText = "update staff_forms_master set File_Content = REPLACE(File_Content,'<form_id>" & old_form_FormID & "</form_id>','<form_id>" & form_FormID & "</form_id>') where unique_form_id = " & form_FormID
  	                        SQLStmtI.CommandType = 1
  	                        Set SQLStmtI.ActiveConnection = conn
  	                        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	                        rsI.Open SQLStmtI  	                
                    ...
                    ELSEIF rs3("Status") = "In-Process" THEN
                     ...
                     'response.Write "in process old form"
                        ...
                        Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                    Set rsI = Server.CreateObject ("ADODB.Recordset")
                       if form_FormType="STAFF_EMR_HELPDESK" or form_FormType="STAFF_TRACK_REPORT" then
  	                    SQLStmtI.CommandText = "exec insert_update_staff_forms_master_emrhelpdesk '" & form_FormType & "'," & form_StaffID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Update'," & form_LinkedFormID & "," & form_ProgramID
                        else
                          SQLStmtI.CommandText = "exec insert_update_staff_forms_master '" & form_FormType & "'," & form_StaffID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Update'," & form_LinkedFormID
                       end if
  	                    SQLStmtI.CommandType = 1
  	                    Set SQLStmtI.ActiveConnection = conn
  	                    SQLStmtI.CommandTimeout = 45 'Timeout per Command
                        ...
                        IF form_DataLock <> "" THEN
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                        SQLStmtI.CommandText = "update staff_forms_master set status = 'Finalized' where unique_form_id = " & form_FormID
  	                        SQLStmtI.CommandType = 1
  	                        Set SQLStmtI.ActiveConnection = conn
  	                        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	                        rsI.Open SQLStmtI
                        ...
                        Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                    Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                    SQLStmtI.CommandText = "exec insert_update_staff_signatures_completed_datalock " & form_FormID & ",'" & form_DataLock & "', 'Update'"
  	                    SQLStmtI.CommandType = 1
  	                    Set SQLStmtI.ActiveConnection = conn
  	                    SQLStmtI.CommandTimeout = 45 'Timeout per Command
                ...
                ELSE
                  ...
                  if form_FormType="STAFF_EMR_HELPDESK" or form_FormType="STAFF_TRACK_REPORT" then
                     ...
                     SQLStmtI.CommandText = "exec insert_update_staff_forms_master '" & form_FormType & "'," & form_StaffID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Add'," & form_LinkedFormID
  	                ...
  	                Set SQLStmtI.ActiveConnection = conn
  	                SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	        '  response.write SQLStmtI.CommandText
  	                rsI.Open SQLStmtI
            ...
            Do while individuals_form_count < individuals_count
                ...
                new_form_string = REPLACE(new_form_string, "process_submit_staff.aspx", "process_submit.aspx")
                ...
                'response.Write "finished for client id = " & cur_individual_client_id & "<br/>"
            ...
            Loop
              ...
              if form_SubmitForm<>"1" then


               Set SQLStmtAlertCheck = Server.CreateObject("ADODB.Command")
  	            Set rsAlertCheck = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtAlertCheck.CommandText = "exec alert_rules_staff_check " & form_FormID & "," & cur_staff_id
             ...
             Set SQLStmtSYLLI = Server.CreateObject("ADODB.Command")
  	            Set rsSYLLI = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtSYLLI.CommandText = "exec insert_entry_into_sylli_log " & form_FormID & "," & form_ProgramID & "," & cur_staff_id
  	            SQLStmtSYLLI.CommandType = 1
  	            Set SQLStmtSYLLI.ActiveConnection = conn
  	         ' response.Write "sql = " & SQLStmtSYLLI.CommandText
             ' response.end
  	            SQLStmtSYLLI.CommandTimeout = 45 'Timeout per Command
  	            rsSYLLI.Open SQLStmtSYLLI
       ...
       if form_SubmitForm = "1" then
          Response.Redirect "http://" & url_org_name & "/upload_files.asp?uid=" & form_FormID & "&fromStaff=1&helpdesk=1&pid=" & form_ProgramID
       end if
