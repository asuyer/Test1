...
if Request.QueryString("ft") <> "" THEN
    ...
    SQLStmt2.CommandText = "select Form_Description from Form_Types_Master where Form_Type = (Select form_type from provider_forms_master where unique_form_id = " & Request.QueryString("uid") & ")"
end if
...
if Request.QueryString("uid") <> "" THEN    
    ...
    Set SQLStmt2 = Server.CreateObject("ADODB.Command")
    Set rs2 = Server.CreateObject ("ADODB.Recordset")
    SQLStmt2.CommandText = "exec get_provider_form_info " & Request.QueryString("uid")
    SQLStmt2.CommandType = 1
    Set SQLStmt2.ActiveConnection = conn
    SQLStmt2.CommandTimeout = 45 'Timeout per Command
    rs2.Open SQLStmt2
    ...
    cur_form_provider = rs2("Provider_ID")
    ...
    SQLStmtA.CommandText = "exec get_staff_provider_form_access " & Request.QueryString("uid") & ",'" & Session("user_name") & "'"
...
<%               
  	    ...
  	    if Request.QueryString("ft") <> "" THEN  	
            ...
            'GET STAFF INFO
            Set SQLStmtProvider = Server.CreateObject("ADODB.Command")
            Set rsProvider = Server.CreateObject ("ADODB.Recordset")
  	        SQLStmtProvider.CommandText = "get_provider_info " & Request.QueryString("prid")
  	        SQLStmtProvider.CommandType = 1
  	        Set SQLStmtProvider.ActiveConnection = conn
  	        SQLStmtProvider.CommandTimeout = 45 'Timeout per Command
 	        rsProvider.Open SQLStmtProvider
 	        
			'response.write(SQLStmtGroup.CommandText)
			
			
			cur_provider_last_name = rsProvider("last_name")
			cur_provider_first_name = rsProvider("first_name")
			cur_provider_name = rsProvider("Provider_Name")
			cur_dob = rsProvider("dob")
			cur_specialty = rsProvider("Specialty")
			cur_is_active = rsProvider("Is_Active")
			cur_external_info = rsProvider("External_Info")
			cur_map_cert = rsProvider("map_cert")
			cur_address = rsProvider("address")
			cur_city = rsProvider("city")
			cur_state = rsProvider("state")
			cur_zip = rsProvider("zip")
			cur_phone = rsProvider("phone")
			cur_fax = rsProvider("fax")
			cur_email = rsProvider("email")
            ...
            elseif InStr(singleline,"<location_name></location_name>") and foundClientName = 0 THEN
             ...
             elseif InStr(singleline,"<providers></providers>") THEN
                  wfile3.Write("<providers>" & cur_provider_name & "</providers>" & chr(13) & chr(10))
             elseif InStr(singleline,"<applicants></applicants>") THEN
                  wfile3.Write("<applicants>" & cur_provider_name & "</applicants>" & chr(13) & chr(10))	
              elseif InStr(singleline,"<completed_by></completed_by>") THEN
                  wfile3.Write("<completed_by>" & cur_provider_name & "</completed_by>" & chr(13) & chr(10))				  
			 elseif InStr(singleline,"<person_first></person_first>") THEN
                  wfile3.Write("<person_first>" & cur_provider_first_name & "</person_first>" & chr(13) & chr(10)) 
		     elseif InStr(singleline,"<provider_first></provider_first>") THEN
                  wfile3.Write("<provider_first>" & cur_provider_first_name & "</provider_first>" & chr(13) & chr(10))		  
			 elseif InStr(singleline,"<person_last></person_last>") THEN
                  wfile3.Write("<person_last>" & cur_provider_last_name & "</person_last>" & chr(13) & chr(10)) 
		     elseif InStr(singleline,"<provider_last></provider_last>") THEN
                  wfile3.Write("<provider_last>" & cur_provider_last_name & "</provider_last>" & chr(13) & chr(10)) 
            elseif InStr(singleline,"<current_date></current_date>") THEN
                  wfile3.Write("<current_date>" & Date & "</current_date>" & chr(13) & chr(10)) 		  
            elseif InStr(singleline,"<address></address>") THEN
                  wfile3.Write("<address>" & cur_address & "</address>" & chr(13) & chr(10)) 
		      elseif InStr(singleline,"<provider_street></provider_street>") THEN
                  wfile3.Write("<provider_street>" & cur_address & "</provider_street>" & chr(13) & chr(10)) 		  
			 elseif InStr(singleline,"<city></city>") THEN
                  wfile3.Write("<city>" & cur_city & "</city>" & chr(13) & chr(10)) 	
		      elseif InStr(singleline,"<provider_city></provider_city>") THEN
                  wfile3.Write("<provider_city>" & cur_city & "</provider_city>" & chr(13) & chr(10)) 			  
			 elseif InStr(singleline,"<contact_state></contact_state>") THEN
                  wfile3.Write("<contact_state>" & cur_state & "</contact_state>" & chr(13) & chr(10)) 
			 elseif InStr(singleline,"<contact_state></contact_state>") THEN
                  wfile3.Write("<contact_state>" & cur_state & "</contact_state>" & chr(13) & chr(10)) 	  
            elseif InStr(singleline,"<zip></zip>") THEN
                  wfile3.Write("<zip>" & cur_zip & "</zip>" & chr(13) & chr(10)) 
			  elseif InStr(singleline,"<provider_zip></provider_zip>") THEN
                  wfile3.Write("<provider_zip>" & cur_zip & "</provider_zip>" & chr(13) & chr(10)) 	  
             elseif InStr(singleline,"<phone></phone>") THEN
                  wfile3.Write("<phone>" & cur_phone & "</phone>" & chr(13) & chr(10)) 				  
              elseif InStr(singleline,"<home_phone></home_phone>") THEN
                  wfile3.Write("<home_phone>" & cur_phone & "</home_phone>" & chr(13) & chr(10))                     
          ...
          elseif InStr(singleline,"<form_id></form_id>") and foundFormID = 0 THEN
           ...
           elseif InStr(singleline,"<form_provider_id></form_provider_id>") THEN
               wfile3.Write("<form_provider_id>" & Request.QueryString("prid") & "</form_provider_id>" & chr(13) & chr(10)) 
