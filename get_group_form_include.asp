...
if Request.QueryString("ft") <> "" THEN
    ...
    SQLStmt2.CommandText = "select Form_Description from Form_Types_Master where Form_Type = (Select form_type from group_forms_master where unique_form_id = " & Request.QueryString("uid") & ")"
end if
...
if Request.QueryString("uid") <> "" THEN    
    ...
    Set SQLStmt2 = Server.CreateObject("ADODB.Command")
    Set rs2 = Server.CreateObject ("ADODB.Recordset")
    SQLStmt2.CommandText = "exec get_group_form_info " & Request.QueryString("uid")
    SQLStmt2.CommandType = 1
    Set SQLStmt2.ActiveConnection = conn
    SQLStmt2.CommandTimeout = 45 'Timeout per Command
    rs2.Open SQLStmt2
    ...
    SQLStmtA.CommandText = "exec get_staff_group_form_access " & Request.QueryString("uid") & ",'" & Session("user_name") & "'"
...
<%               
  	    ...
  	    if Request.QueryString("ft") <> "" THEN  	    
            ...
            model_end_found = 0
             ...
             if Request.QueryString("ft") = "FIRE_GROUP" then
            
                 Set SQLStmtMembers2 = Server.CreateObject("ADODB.Command")
                Set rsMembers2 = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtMembers2.CommandText = "get_fire_drill_group " & Request.QueryString("gid")
  	            SQLStmtMembers2.CommandType = 1
  	            Set SQLStmtMembers2.ActiveConnection = conn
  	            SQLStmtMembers2.CommandTimeout = 45 'Timeout per Command
  	          '  response.Write "sql = " & SQLStmtMembers.CommandText
 	            rsMembers2.Open SQLStmtMembers2
               
                Do Until rsMembers2.EOF
                memeber_count = rsMembers2("member_count")
                description = rsMembers2("Description")
                rsMembers2.Movenext
                loop
            ...
            Set rsGroup = Server.CreateObject ("ADODB.Recordset")
  	        SQLStmtGroup.CommandText = "get_group_info " & Request.QueryString("gid")
 	        ...
 	        cur_group_name = rsGroup("Group_Name")
 	        cur_group_member_count = rsGroup("group_member_count")
            ...
            elseif InStr(singleline,"<group_name></group_name>") and foundClientName = 0 THEN
               wfile3.Write("<group_name>" & cur_group_name & "</group_name>" & chr(13) & chr(10)) 
               
            elseif InStr(singleline,"<num_attendees></num_attendees>") and foundClientName = 0 and Request.QueryString("ft") <> "FIRE_GROUP" THEN
               wfile3.Write("<num_attendees>" & cur_group_member_count & "</num_attendees>" & chr(13) & chr(10)) 
            
           
           
            
            elseif InStr(singleline,"<num_attendees></num_attendees>") and  Request.QueryString("ft") = "FIRE_GROUP" THEN
               wfile3.Write("<num_attendees>" & memeber_count & "</num_attendees>" & chr(13) & chr(10)) 
               
            elseif InStr(singleline,"<ind_loc></ind_loc>") and  Request.QueryString("ft") = "FIRE_GROUP" THEN
               wfile3.Write("<ind_loc>" & description & "</ind_loc>" & chr(13) & chr(10)) 
               
            '**********START INDIVIDUALS IN THE GROUP SECTION, LOOP FOR EACH PERSON IN THE GROUP
        
             
         elseif InStr(singleline,"<individuals_section>") <> 0 and Request.QueryString("ft") <> "FIRE_GROUP" THEN
             
                'items that will always be in an indivual section for a group form are
                ...
                individual_section_contents = ""
                
                wfile3.write(singleline & chr(13) & chr(10))
                singleline = wfile.readline
                                
                if InStr(singleline,"<individual_person>") <> 0 THEN     
                
                    'LOOP UNTIL YOU FIND THE LAST LINE OF INDIVIDUAL PERSON AND BUILD THE TEMPLATE PERSON SECTION STRING
                    Do Until InStr(singleline,"</individual_person>") <> 0
                        individual_section_contents = individual_section_contents & singleline & chr(13) & chr(10)
                        singleline = wfile.readline
                    Loop
                ...
                Set SQLStmtMembers = Server.CreateObject("ADODB.Command")
                Set rsMembers = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtMembers.CommandText = "get_group_members " & Request.QueryString("gid")
  	            SQLStmtMembers.CommandType = 1
  	            Set SQLStmtMembers.ActiveConnection = conn
  	            SQLStmtMembers.CommandTimeout = 45 'Timeout per Command
  	           ' response.Write "sql = " & SQLStmtMembers.CommandText
 	            rsMembers.Open SQLStmtMembers

                Do Until rsMembers.EOF
                
                    cur_group_program = rsMembers("Responsible_Program")
                    cur_group_program_name = rsMembers("Responsible_Program_Name")
                    cur_group_organizer = rsMembers("Organizer")
                    cur_group_organizer_name = rsMembers("Organizer_Name")
                    cur_group_name = rsMembers("Group_Name")
                    
                    individual_split = Split(individual_section_contents, vbCrLf)             
                    found_goal_table_start = 0
                    found_goal_table_end = 0
                                        
                    for each cur_split_line in individual_split
                        'CHECK FOR INDIVIDUAL FIELDS TO PREPOP                            
                        if InStr(cur_split_line,"<person_first>") <> 0 THEN     
                             wfile3.Write("<person_first>" & rsMembers("first_name") & "</person_first>" & chr(13) & chr(10)) 
                        elseif InStr(cur_split_line,"<person_middle>") <> 0 THEN 
                             wfile3.Write("<person_middle>" & rsMembers("middle_name") & "</person_middle>" & chr(13) & chr(10)) 
                        elseif InStr(cur_split_line,"<person_last>") <> 0 THEN 
                             wfile3.Write("<person_last>" & rsMembers("last_name") & "</person_last>" & chr(13) & chr(10))  
                        elseif InStr(cur_split_line,"<rec_num>") <> 0 THEN 
                             wfile3.Write("<rec_num>" & rsMembers("client_number") & "</rec_num>" & chr(13) & chr(10)) 
                        elseif InStr(cur_split_line,"<dob>") <> 0 THEN 
                             wfile3.Write("<dob>" & rsMembers("dob") & "</dob>" & chr(13) & chr(10)) 
                        elseif InStr(cur_split_line,"<client_id>") <> 0 THEN 
                             wfile3.Write("<client_id>" & rsMembers("client_id") & "</client_id>" & chr(13) & chr(10)) 
                        elseif InStr(cur_split_line,"<date_of_service>") <> 0 THEN
                            
                            if rsMembers("Meeting_Date") <> "" THEN
                                cur_meeting_date = rsMembers("Meeting_Date")
                            end if
                            
                            'response.Write "meeting date = " & cur_meeting_date
                            
                            split_date = Split(LEFT(FormatDateTime(cur_meeting_date,2) ,10),"/")
                            dos_date_day = RIGHT("0" & split_date(1),2)
                            dos_date_month = RIGHT("0" & split_date(0),2)
                            dos_date_year = RIGHT(split_date(2), 4)
                        
                            wfile3.Write("<date_of_service>" & dos_date_year & "-" & dos_date_month & "-" & dos_date_day & "</date_of_service>" & chr(13) & chr(10))
                        
                        elseif InStr(cur_split_line,"<start_time>") <> 0 THEN 
                             wfile3.Write("<start_time>" & rsMembers("start_time") & "</start_time>" & chr(13) & chr(10)) 
                             
                        elseif InStr(cur_split_line,"<stop_time>") <> 0 THEN 
                             wfile3.Write("<stop_time>" & rsMembers("stop_time") & "</stop_time>" & chr(13) & chr(10))      
                            
                        elseif InStr(cur_split_line,"<provider_number>") <> 0 THEN
                            wfile3.Write("<provider_number>" & rsMembers("organizer") & "</provider_number>" & chr(13) & chr(10)) 
                        
                        elseif InStr(cur_split_line,"<goal_table>") <> 0 THEN
            
                            found_goal_table_start = 1
                            
                            wfile3.Write (cur_split_line & chr(13) & chr(10))
                                                
                            'set goal and objective counters for loop tags
  	                        goal_counter = 1
  	                        goal_row_counter = 1
  	                        max_obj_row_count = 1  	     
              	            
  	                       cbhi_ignores_needs_older_than_one_year = 0       
              	            
  	                        'sql query to find all goals/objectives that should reflect on this form
  	                        Set SQLStmtG2 = Server.CreateObject("ADODB.Command")
  	                        Set rsG2 = Server.CreateObject ("ADODB.Recordset")
  	                        'GOALS FOR CBHI GOLDEN THREAD
  	                        IF Request.QueryString("ft") = "IHTPN" or Request.QueryString("ft") = "TMSN" or Request.QueryString("ft") = "FSTSN" or Request.QueryString("ft") = "IHBSN"  THEN
  	                           SQLStmtG2.CommandText = "get_cbhi_goals_for_client_and_form " & rsMembers("client_id") & "," & notes_require_finalized_iap & "," & cbhi_ignores_needs_older_than_one_year & "," & Request.QueryString("lfid")
  	                        ELSE
  	                            if Request.QueryString("lfid") <> "" THEN
  	                                SQLStmtG2.CommandText = "get_goals_from_form " & Request.QueryString("lfid")
  	                            else
  	                                SQLStmtG2.CommandText = "get_goals_for_client " & rsMembers("client_id")
  	                            end if
  	                        END IF
  	                        
  	                        SQLStmtG2.CommandType = 1
  	                        Set SQLStmtG2.ActiveConnection = conn
  	                       'response.Write "sql = " & SQLStmtG2.CommandText
  	                        SQLStmtG2.CommandTimeout = 45 'Timeout per Command
  	                        rsG2.Open SQLStmtG2
              	            
  	                        Do Until rsG2.EOF
              	            
      	                        cur_goal_desc = rsG2("Goal_Desc")
              	                
  	                            if  request.QueryString("ft") = "CBFSSNWE" or request.QueryString("ft") = "CBFSWSN"  THEN
  	                                'NEED TO USE GOAL NUM NOT GOAL COUNTER FOR THIS FORM
  	                                goal_counter = rsG2("Goal_Num")
  	                            end if
              	                        
                                goal_row_counter = goal_row_counter + 1
                            
                                max_obj_row_count = 1
                                
                                wfile3.Write ("<row>" & chr(13) & chr(10))
                                wfile3.Write ("    <goal_check></goal_check>" & chr(13) & chr(10))
                                wfile3.Write ("    <goal_num>" & goal_counter & "</goal_num>" & chr(13) & chr(10))                                                
                                wfile3.Write ("    <goal_text>" & cur_goal_desc & "</goal_text>" & chr(13) & chr(10)) 
              	                    
  	                            objective_row_counter  = 1	                                    
  	                            objective_counter = 1
              	                
  	                            Set SQLStmtO2 = Server.CreateObject("ADODB.Command")
  	                            Set rsO2 = Server.CreateObject ("ADODB.Recordset")
  	                            IF Request.QueryString("ft") = "IHTPN" or Request.QueryString("ft") = "TMSN" or Request.QueryString("ft") = "FSTSN" or Request.QueryString("ft") = "IHBSN"  THEN
  	                                SQLStmtO2.CommandText = "get_cbhi_objectives_for_goal " & rsG2("Goal_ID") & ",'Active'"
  	                            ELSE
  	                                SQLStmtO2.CommandText = "get_objectives_for_goal " & rsG2("Goal_ID") & ",'Active'"
  	                            END IF
  	                            SQLStmtO2.CommandType = 1
  	                            Set SQLStmtO2.ActiveConnection = conn
  	                           'Response.Write("sql = " & SQLStmtO2.CommandText)
  	                            SQLStmtO2.CommandTimeout = 45 'Timeout per Command
  	                            rsO2.Open SQLStmtO2
                  	            
      	                        wfile3.Write ("    <objectives>" & chr(13) & chr(10))
                  	            
  	                            Do Until rsO2.EOF  	                
  	                                cur_obj_desc = rsO2("objective_narrative")
              	                    
  	                                if  request.QueryString("ft") = "CBFSSNWE" or request.QueryString("ft") = "CBFSWSN" THEN
  	                                    'NEED TO USE GOAL NUM NOT GOAL COUNTER FOR THIS FORM
  	                                    objective_counter = rsO2("obj_num")
  	                                end if
              	                    
  	                                wfile3.Write ("        <objective>" & chr(13) & chr(10))
  	                                wfile3.Write ("            <objective_check></objective_check>" & chr(13) & chr(10))
  	                                wfile3.Write ("            <objective_num>" & objective_counter & "</objective_num>" & chr(13) & chr(10))                                        
                                    wfile3.Write ("            <objective_text>" & cur_obj_desc & "</objective_text>" & chr(13) & chr(10))
                                    if request.QueryString("ft") = "MPNS" then
                                         wfile3.Write ("            <objective_status></objective_status>" & chr(13) & chr(10))
                                    end if
                                    wfile3.Write ("                <interventions>" & chr(13) & chr(10))
                    
  	                                'if request.QueryString("ft") = "CBFSSN" or request.QueryString("ft") = "CBFSSNWE" or request.QueryString("ft") = "CBFSWSN"  THEN
  	                                '****NEW INTERVENTIONS SECTION  	                    
              	                    
  	                                    interventions_counter = 0
              	                    
  	                                    intervention_1 = rsO2("interventions_r1")
  	                                    intervention_2 = rsO2("interventions_r2")
  	                                    intervention_3 = rsO2("interventions_r3")
      	                                intervention_4 = rsO2("interventions_r4")
  	                                    intervention_5 = rsO2("interventions_r5")
  	                                    intervention_6 = rsO2("interventions_r6")
              	                        
  	                                    if intervention_1 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>1</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_1 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 1
  	                                    end if                       
  	                                    if intervention_2 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>2</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_2 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 2
  	                                    end if  	                        
  	                                    if intervention_3 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>3</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_3 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 3
  	                                    end if 	         
  	                                    if intervention_4 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>1</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_4 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 1
  	                                    end if                       
  	                                    if intervention_5 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>2</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_5 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 2
  	                                    end if  	                        
  	                                    if intervention_6 <> "" THEN
  	                                        wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
  	                                        wfile3.Write ("                        <intervention_num>3</intervention_num>" & chr(13) & chr(10))
                                            wfile3.Write ("                        <intervention_text>" & intervention_6 & "</intervention_text>" & chr(13) & chr(10))
                                            wfile3.Write ("                    </intervention>" & chr(13) & chr(10))
                                            interventions_counter = 3
  	                                    end if
  	                                    wfile3.Write ("                    <intervention>" & chr(13) & chr(10))
                                        wfile3.Write ("                        <intervention_check></intervention_check>" & chr(13) & chr(10))
                                        wfile3.Write ("                        <intervention_num></intervention_num>" & chr(13) & chr(10))
                                        wfile3.Write ("                        <intervention_text></intervention_text>" & chr(13) & chr(10))
                                        wfile3.Write ("                    </intervention>" & chr(13) & chr(10))           
  	                               '****END OF INTERVENTIONS SECTION
  	                               'END IF
  	                     '             	     
  	                                wfile3.Write ("                </interventions>" & chr(13) & chr(10))
  	                                wfile3.Write ("        </objective>" & chr(13) & chr(10))
              	                      	     
  	                                objective_row_counter = objective_row_counter + 1           
                                ...
                                wfile3.Write ("        <objective>" & chr(13) & chr(10))
                                wfile3.Write ("            <objective_check></objective_check>" & chr(13) & chr(10))
                                wfile3.Write ("            <objective_num></objective_num>" & chr(13) & chr(10))                                        
                                wfile3.Write ("            <objective_text></objective_text>" & chr(13) & chr(10))
                                if request.QueryString("ft") = "MPNS" then
                                     wfile3.Write ("            <objective_status></objective_status>" & chr(13) & chr(10))
                                end if
                                wfile3.Write ("                <interventions>" & chr(13) & chr(10))
                                ...
                                goal_counter = goal_counter + 1
  	                            ...
  	                            if max_obj_row_count < objective_row_counter THEN
                                    max_obj_row_count = objective_row_counter
                                end if
                                
  	                        rsG2.MoveNext
                            Loop           
                            
                           'Fixes count when only one goal
                           'wfile3.Write(cur_split_line & chr(13) & chr(10))
                            
                            'cur_split_line = wfile.readline
                            'if InStr(cur_split_line,"<goal_check>") = 0 then
                            '    wfile3.Write ("    <goal_check></goal_check>" & chr(13) & chr(10))
                            '    wfile3.Write ("    <goal_num></goal_num>" & chr(13) & chr(10))                                                
                            '    wfile3.Write ("    <goal_text></goal_text>" & chr(13) & chr(10)) 
  	                        'end if
  	                        
                            'do until InStr(cur_split_line,"</goal_table>")
                            '    wfile3.Write(cur_split_line & chr(13) & chr(10))
                            '    cur_split_line = wfile.readline
                            'loop     
                            'wfile3.Write(cur_split_line & chr(13) & chr(10))
                            '****** END NEW GOALS POPULATION ******
                        
                        elseif InStr(cur_split_line,"</goal_table>") <> 0 THEN
                            found_goal_table_end = 1
                                                        
                            'SPIT OUT TEMPLATE ROW NOW
                            wfile3.write("<row>" & chr(13) & chr(10))
                            wfile3.write("    <goal_check></goal_check>" & chr(13) & chr(10))
                            wfile3.write("    <goal_num></goal_num>" & chr(13) & chr(10))
                            wfile3.write("    <goal_text></goal_text>" & chr(13) & chr(10))
                        ...
                        elseif found_goal_table_start = 1 and found_goal_table_end = 0 THEN
                            'skip this row    
                        else                           
                            wfile3.write(cur_split_line & chr(13) & chr(10))
                        end if
            ...
            '**********END INDIVIDUALS IN THE GROUP SECTION, LOOP FOR EACH PERSON IN THE GROUP
           
           
            
            'elseif InStr(singleline,"<person_first></person_first>") THEN
            '   foundClientFName = 1
            '   wfile3.Write(Replace(singleline, "<person_first></person_first>", "<person_first>" & rs2("first_name") & "</person_first>") & chr(13) & chr(10)) 
            'elseif InStr(singleline,"<person_middle></person_middle>") THEN
            '  foundClientMName = 1
            '   wfile3.Write(Replace(singleline, "<person_middle></person_middle>", "<person_middle>" & rs2("middle_name") & "</person_middle>") & chr(13) & chr(10)) 
            'elseif InStr(singleline,"<person_last></person_last>") THEN
            '   foundClientLName = 1
            '   wfile3.Write(Replace(singleline, "<person_last></person_last>", "<person_last>" & rs2("last_name") & "</person_last>") & chr(13) & chr(10))                    
            'elseif InStr(singleline,"<client_id></client_id>") and foundClientID = 0 THEN
            '   foundClientID = 1
            '   wfile3.Write(Replace(singleline, "<client_id></client_id>", "<client_id>" & Request.QueryString("cid") & "</client_id>") & chr(13) & chr(10))
            
            'PIF DEMO FIELDS BEGIN****************************************************
            'elseif InStr(singleline,"<dob></dob>") <> 0 and (foundClientDOB = 0 or Request.QueryString("ft") = "ESPACA" or Request.QueryString("ft") = "ESPCCA") and rs2("dob") <> "1/1/1900" THEN
            '   foundClientDOB = 1
            '   wfile3.Write(Replace(singleline, "<dob></dob>", "<dob>" & rs2("dob") & "</dob>") & chr(13) & chr(10))
            
             'FIX FOR MISNAMED FIELD
            'elseif InStr(singleline,"<organization_name1></organization_name1>") <> 0 and Request.QueryString("ft") = "PPPN" and rs2("dob") <> "1/1/1900" THEN
            '   foundClientDOB = 1
            '   wfile3.Write(Replace(singleline, "<organization_name1></organization_name1>", "<organization_name1>" & rs2("dob") & "</organization_name1>") & chr(13) & chr(10))
               
            'elseif InStr(singleline,"<ssn></ssn>") <> 0 and (foundClientSSN = 0 or Request.QueryString("ft") = "ESPACA" or Request.QueryString("ft") = "ESPCCA") THEN
            '      foundClientSSN = 1
            '      wfile3.Write(Replace(singleline, "<ssn></ssn>", "<ssn>" & rs2("ssn") & "</ssn>") & chr(13) & chr(10))
          ...
          elseif InStr(singleline,"<form_id></form_id>") and foundFormID = 0 THEN
           ...
           elseif InStr(singleline,"<group_id></group_id>") THEN
               wfile3.Write("<group_id>" & Request.QueryString("gid") & "</group_id>" & chr(13) & chr(10)) 
           ...
           ' elseif InStr(singleline,"<rec_num></rec_num>") and (foundRecNum = 0 or Request.QueryString("ft") = "ESPACA" or Request.QueryString("ft") = "ESPCCA") THEN
           '    foundRecNum = 1
           '    if use_dmh_acct_num_for_recnum = 1 and Request.QueryString("ft") <> "CHTF" THEN
           '         form_rec_num_id = rs2("dmh_account_number")
           '    elseif use_cmhc_id_for_recnum = 1 THEN
           '         form_rec_num_id = rs2("client_number")'rs2("external_info")
           '    else
           '         form_rec_num_id = Request.QueryString("cid")
           '    end if
               
          '     response.Write "rec num section rec num = " & form_rec_num_id
               
               'if use_cmhc_id_for_recnum = 1 THEN
               '     form_rec_num_id = rs2("external_info")
               'else
               '     form_rec_num_id = Request.QueryString("cid")
               'end if
            ...
            elseif InStr(singleline,"<dr_first_allergies>") <> 0 THEN 'ALL OTHER ALLERGIES HERE
                    ...
                    Set rsMeds = Server.CreateObject ("ADODB.Recordset")
  	                ...
  	                if medrev_meds_count = 0 THEN
  	                   ' wfile3.Write ("<row>" & chr(13) & chr(10))
  	                   '     wfile3.Write ("<allergy_type>Dr_First</allergy_type>" & chr(13) & chr(10))
                       '     wfile3.Write ("<allergy_name></allergy_name>" & chr(13) & chr(10))
                       '     wfile3.Write ("<allergy_reaction></allergy_reaction>" & chr(13) & chr(10))
                       '     wfile3.Write ("<allergy_kind></allergy_kind>" & chr(13) & chr(10))
                       '     wfile3.Write ("<allergy_onset_date></allergy_onset_date>" & chr(13) & chr(10))
                       '     wfile3.Write ("<allergy_status></allergy_status>" & chr(13) & chr(10))
                       ' wfile3.Write ("</row>" & chr(13) & chr(10))
  	                end if
            ...
            elseif InStr(singleline,"<dr_first_meds_table>") <> 0 THEN 'DR FIRST MEDS HERE
  	         ...
  	         wfile3.Write (singleline & chr(13) & chr(10))
  	            ...
  	            Set SQLStmtMembers = Server.CreateObject("ADODB.Command")
                Set rsMembers = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtMembers.CommandText = "get_fire_drill_group_members " & Request.QueryString("gid")
  	            SQLStmtMembers.CommandType = 1
  	            Set SQLStmtMembers.ActiveConnection = conn
  	            SQLStmtMembers.CommandTimeout = 45 'Timeout per Command
  	          '  response.Write "sql = " & SQLStmtMembers.CommandText
 	            rsMembers.Open SQLStmtMembers
 	        '    wfile3.Write ("<!--" & SQLStmtMembers.CommandText & "-->" & chr(13) & chr(10))
  	                Do Until rsMembers.EOF
  	                
  	                
  	                  wfile3.Write ("<individual_person>" & chr(13) & chr(10))
                      wfile3.Write ("<client_id>" & rsMembers("client_id") & "</client_id>" & chr(13) & chr(10))
  	                  wfile3.Write ("<name>" & rsMembers("member_name") & "</name>" & chr(13) & chr(10))
  	                  wfile3.Write ("<na_other></na_other>" & chr(13) & chr(10))
  	                  wfile3.Write ("<present></present>" & chr(13) & chr(10))
  	                  wfile3.Write ("<ad_other></ad_other>" & chr(13) & chr(10))
  	                  wfile3.Write ("<exit_time></exit_time>" & chr(13) & chr(10))
  	                  wfile3.Write ("<support_level></support_level>" & chr(13) & chr(10))
  	                  wfile3.Write ("<exit_time_hidden></exit_time_hidden>" & chr(13) & chr(10))
  	                  wfile3.Write ("<exit_time_hidden1></exit_time_hidden1>" & chr(13) & chr(10))
  	                  wfile3.Write ("<non_ambulatory></non_ambulatory>" & chr(13) & chr(10))
  	                  wfile3.Write ("<adaptive_device></adaptive_device>" & chr(13) & chr(10))
  	                  wfile3.Write ("<asleep></asleep>" & chr(13) & chr(10))
  	                  wfile3.Write ("<person_location></person_location>" & chr(13) & chr(10))
  	                ...
  	                Do Until InStr(singleline, "</individuals_section>") <> 0
  	                    singleline=wfile.readline 
  	                Loop    
            ...
            elseif InStr(singleline,"<current_past_meds_table>") <> 0 THEN 
                    ...
                    Set SQLStmtMeds = Server.CreateObject("ADODB.Command")
                    Set rsMeds = Server.CreateObject ("ADODB.Recordset")
                    'if Request.QueryString("ft") = "ESPACA" THEN
                    '    SQLStmtMeds.CommandText = "exec get_current_meds_for_client_by_type " & Request.QueryString("cid") & ",'All_Other'"
                    'else
        ...
        '***********************************************************************************
            ...
            elseif InStr(singleline, "<!-- INSERT DYNAMIC FORM RULES HERE -->") THEN
                ...
                rsRules.MoveNext
                Loop 
             elseif InStr(singleline,"<xforms:instance id=""Staff_List"" xmlns="""">") THEN
                    wfile3.Write (singleline & chr(13) & chr(10))
                    singleline=wfile.readline 
                    wfile3.Write (singleline & chr(13) & chr(10))
                    ...
                    Set SQLStmtV = Server.CreateObject("ADODB.Command")
  	                Set rsV = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtV.CommandText = "select staff_id, last_name + ', ' + first_name as staff_name from staff_master where is_active = 1 order by last_name"
  	                ' where program_id = '" & Request.QueryString("pid") & "') order by staff_name "
  	                SQLStmtV.CommandType = 1
  	                Set SQLStmtV.ActiveConnection = conn
  	                SQLStmtV.CommandTimeout = 45 'Timeout per Command
  	                rsV.Open SQLStmtV
  	                ...
  	                Do Until rsV.EOF
                         ...
                         wfile3.Write ("<choice value=""" & rsV("staff_id") & """>" & rsv("staff_name") & "</choice>" & chr(13) & chr(10))
