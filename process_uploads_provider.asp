...
<% 
      ...
      if precheck_form_value <> "" and form_FormID <> "" and form_FormID <> "-1" THEN
  	    ...
  	    SQLStmt2.CommandText = "exec does_provider_form_exist " & rsI("form_value")
        ...
        'IF FORM ID IS FOUND IN SYSTEM        
  	    IF rs2("form_id_exists") = "Yes" THEN
	        ...
	        SQLStmt3.CommandText = "exec get_provider_form_info " & rsI("form_value")
    	    ...
    	    checked_form_provider = rs3("Provider_ID")
       ...
       if foundSameFormHashMatch = 1 THEN 'THIS MEANS THE SAME FORM WAS SUBMITTED WITH NO SIGNED CHANGES ON IT, PIF AND ESP COMPS ALLOW UNSIGNED CHANGES SO SAVE FORM CONTENT ONLY
            ...
            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	        SQLStmtI.CommandText = "exec update_existing_provider_form_content " & checked_form_id & ",'" & new_xml_Contents & "','" & Session("user_name") & "'"
  	        SQLStmtI.CommandType = 1
  	        Set SQLStmtI.ActiveConnection = conn
  	        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	        rsI.Open SQLStmtI
            ...
            do while not wfile.AtEndOfStream 
                ...
                if InStr(singleline,"<form_provider_id>") and foundProviderID = 0 THEN
                                    
                    foundProviderID = 1
                    form_id_tag_start = InStr(singleline,"<form_provider_id>")
                    form_id_tag_end = InStr(singleline,"</form_provider_id>")
                    total_tag_length = form_id_tag_end - form_id_tag_start
                    form_ProviderID = Mid(singleline,(form_id_tag_start+18),(total_tag_length-18))
            ...
            'IF FORM ID IS EMPTY 
                ...
                Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	            Set rsI = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtI.CommandText = "exec insert_update_provider_forms_master '" & form_FormType & "'," & form_ProviderID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Add'," & form_LinkedFormID
  	            SQLStmtI.CommandType = 1
  	            Set SQLStmtI.ActiveConnection = conn
  	            SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	            rsI.Open SQLStmtI
                ...
                Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	            Set rsI = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtI.CommandText = "exec insert_update_provider_signatures_completed_datalock " & form_FormID & ",'" & form_DataLock & "', 'Add'"
  	            SQLStmtI.CommandType = 1
  	            Set SQLStmtI.ActiveConnection = conn
  	            SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	            rsI.Open SQLStmtI
            ...
            ELSE                
                ...
                Set SQLStmt2 = Server.CreateObject("ADODB.Command")
  	            Set rs2 = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmt2.CommandText = "exec does_provider_form_exist " & idToFind 
  	            SQLStmt2.CommandType = 1
  	            Set SQLStmt2.ActiveConnection = conn
  	            SQLStmt2.CommandTimeout = 45 'Timeout per Command
  	            'response.Write "sql = " & SQLStmt2.CommandText
  	            ...
  	            IF rs2("form_id_exists") = "Yes" THEN
                    ...
                    Set SQLStmt3 = Server.CreateObject("ADODB.Command")
  	                Set rs3 = Server.CreateObject ("ADODB.Recordset")
	                SQLStmt3.CommandText = "exec get_provider_form_info " & idToFind
                    SQLStmt3.CommandType = 1
                    Set SQLStmt3.ActiveConnection = conn
                    SQLStmt3.CommandTimeout = 45 'Timeout per Command
                    'response.Write "sql = " & SQLStmt3.CommandText
                    rs3.Open SQLStmt3
  	                ...
  	                IF rs3("Status") = "Finalized" THEN
                        ...
                        ELSE
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                        SQLStmtI.CommandText = "exec insert_update_provider_forms_master '" & form_FormType & "'," & form_ProviderID & "," & form_FormID & "," & old_form_FormID & ",'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','" & new_form_child_status & "','Add'," & form_LinkedFormID
  	                        SQLStmtI.CommandType = 1
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                        SQLStmtI.CommandText = "update provider_forms_master set File_Content = REPLACE(File_Content,'<form_id>" & old_form_FormID & "</form_id>','<form_id>" & form_FormID & "</form_id>') where unique_form_id = " & form_FormID
  	                        SQLStmtI.CommandType = 1
  	                        Set SQLStmtI.ActiveConnection = conn
  	                        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	                        rsI.Open SQLStmtI  	                
                    ...
                    ELSEIF rs3("Status") = "In-Process" THEN
                     ...
                     'response.Write "in process old form"
                        ...
                        Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                    Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                    SQLStmtI.CommandText = "exec insert_update_provider_forms_master '" & form_FormType & "'," & form_ProviderID & "," & form_FormID & ",0,'" & new_file_Contents & "','" & Session("user_name") & "','" & Session("user_name") & "','In-Process','Update'," & form_LinkedFormID
  	                    SQLStmtI.CommandType = 1
  	                    Set SQLStmtI.ActiveConnection = conn
  	                    SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	                    rsI.Open SQLStmtI
                        ...
                        IF form_DataLock <> "" THEN
                            ...
                            Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                        Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                        SQLStmtI.CommandText = "update provider_forms_master set status = 'Finalized' where unique_form_id = " & form_FormID
  	                        SQLStmtI.CommandType = 1
  	                        Set SQLStmtI.ActiveConnection = conn
  	                        SQLStmtI.CommandTimeout = 45 'Timeout per Command
  	                        rsI.Open SQLStmtI
                        ...
                        Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	                    Set rsI = Server.CreateObject ("ADODB.Recordset")
  	                    SQLStmtI.CommandText = "exec insert_update_provider_signatures_completed_datalock " & form_FormID & ",'" & form_DataLock & "', 'Update'"
  	                    SQLStmtI.CommandType = 1
  	                    Set SQLStmtI.ActiveConnection = conn
  	                    SQLStmtI.CommandTimeout = 45 'Timeout per Command
            ...
            Do while individuals_form_count < individuals_count
                ...
                new_form_string = REPLACE(new_form_string, "process_submit_provider.aspx", "process_submit.aspx")
