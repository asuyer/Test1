<%
  ...
  '	response.write "SQL = " & SQLStmt2.CommandText
    ...
    cur_login_staff_name = rs2("First_Name") & " " & rs2("Last_Name")
    ...
    if cur_staff_id=7 or cur_staff_id=9 or cur_staff_id=1335 or cur_staff_id=1336 or emr_staff=1 then
    agency_program_id =-1
    end if
...
if Request.QueryString("ft") <> "" THEN
    ...
    SQLStmt2.CommandText = "select Form_Description from Form_Types_Master where Form_Type = (Select form_type from staff_forms_master where unique_form_id = " & Request.QueryString("uid") & ")"
end if
...
if Request.QueryString("uid") <> "" THEN    
    ...
    Set SQLStmt2 = Server.CreateObject("ADODB.Command")
    Set rs2 = Server.CreateObject ("ADODB.Recordset")
    SQLStmt2.CommandText = "exec get_staff_form_info " & Request.QueryString("uid")
    SQLStmt2.CommandType = 1
    Set SQLStmt2.ActiveConnection = conn
    SQLStmt2.CommandTimeout = 45 'Timeout per Command
    rs2.Open SQLStmt2
  ...
  Set SQLStmtPre = Server.CreateObject("ADODB.Command")
    ...
    SQLStmtA.CommandText = "exec get_staff_staff_form_access " & Request.QueryString("uid") & ",'" & Session("user_name") & "'"
    ...
    form_access_level = rsA("Access_Level")
  	...
  	'SQLStmtS.CommandText = "exec get_signatures_completed_info_for_staff_and_form " & Request.QueryString("uid") & ",'" & Session("user_name") & "'"
  	'SQLStmtS.CommandType = 1
  	'Set SQLStmtS.ActiveConnection = conn
  	'SQLStmtS.CommandTimeout = 45 'Timeout per Command
  	'response.Write "sql = " & SQLStmtS.CommandText
  	'rsS.Open SQLStmtS   
  	
  	'Do Until rsS.EOF
  	'        if rsS("Completed_Type") = "Person Served" and rsS("Completed_Signer") <> "" THEN
  	'            cur_person_signer = rsS("Completed_Signer")
  	'            cur_person_sign_date = rsS("Completed_Signature_Date") 
  	'            cur_person_signer_name = rsS("Completed_Signer_Name")   	            
  	'        elseif rsS("Completed_Type") = "Provider" and rsS("Completed_Signer") <> "" THEN
  	'            cur_provider_signer = rsS("Completed_Signer")
  	'            cur_provider_sign_date = rsS("Completed_Signature_Date") 
  	'            cur_provider_signer_name = rsS("Completed_Signer_Name")   	            
  	'            cur_provider_sign_position = rsS("Completed_Signature_Position")  	            
  	'        elseif rsS("Completed_Type") = "MD" and rsS("Completed_Signer") <> "" THEN
  	'            cur_md_signer = rsS("Completed_Signer")
  	'            cur_md_sign_date = rsS("Completed_Signature_Date") 
  	'            cur_md_signer_name = rsS("Completed_Signer_Name")  	            
  	'            cur_md_sign_position = rsS("Completed_Signature_Position")
  	'        elseif rsS("Completed_Type") = "Supervisor" and rsS("Completed_Signer") <> "" THEN
  	'            cur_supervisor_signer = rsS("Completed_Signer")
  	'            cur_supervisor_sign_date = rsS("Completed_Signature_Date") 
  	'            cur_supervisor_signer_name = rsS("Completed_Signer_Name")   	            
  	'        elseif rsS("Completed_Type") = "Parent Guardian" and rsS("Completed_Signer") <> "" THEN
  	'            cur_parent_signer = rsS("Completed_Signer")
  	'            cur_parent_sign_date = rsS("Completed_Signature_Date") 
  	'            cur_parent_signer_name = rsS("Completed_Signer_Name")   	            
  	'            cur_supervisor_sign_position = rsS("Completed_Signature_Position")
  	'        end if
  	'    rsS.MoveNext
  	'Loop
    ...
    Set SQLStmtI = Server.CreateObject("ADODB.Command")
  	...
  	SQLStmtI.CommandText = "exec insert_staff_transaction_history " & Request.QueryString("uid") & ",'" & Session("user_name") & "','View',''"
    ...
    Set SQLStmtC = Server.CreateObject("ADODB.Command")
  	...
  	SQLStmtC.CommandText = "get_staff_info " & cur_form_staff
  	SQLStmtC.CommandType = 1
  	Set SQLStmtC.ActiveConnection = conn
  	SQLStmtC.CommandTimeout = 45 'Timeout per Command
  	rsC.Open SQLStmtC
    ...
    do while not wfile.AtEndOfStream 
       ...
       if InStr(singleline,"<provider_signer></provider_signer>") and foundProvSigner = 0 THEN
        ...
        elseif InStr(singleline,"<supervisor_name></supervisor_name>") and foundSupervisorName = 0 THEN
            ...
            wfile3.Write(Replace(singleline, "<supervisor_name></supervisor_name>", "<supervisor_name>" & cur_supervisor_signer_name & "</supervisor_name>") & chr(13) & chr(10))
              ...
              elseif InStr(singleline,"<mandatory compute=""Is_Edit.value == '1' ? 'on' : 'off'"">on</mandatory>") <> 0  THEN
            wfile3.Write("<mandatory compute=""Is_Edit.value == '11111' ? 'on' : 'off'"">on</mandatory>" & chr(13) & chr(10))  
        ...
        elseif InStr(singleline,"<allergies></allergies>") <> 0 and cur_client_allergies <> "" and cur_form_status <> "Finalized" THEN
         ...
         elseif InStr(singleline,"<!--START EXCLUDE DD-->") <> 0 and agency_program_id <>-1 THEN
            
              
            
              
                
                Do Until InStr(singleline,"<!--END EXCLUDE DD-->") <> 0
                    singleline=wfile.readline 
  	            Loop  


    
       elseif InStr(singleline, "<!--START ATTACHMENT VALUE-->") and Request.QueryString("uid") <> "" THEN
               ...
               wfile3.Write (singleline & chr(13) & chr(10))
                singleline=wfile.readline 
                  ...
                  Set SQLStmtCM = Server.CreateObject("ADODB.Command")
  	                Set rsCM = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtCM.CommandText = "select  document_name from Documents_Master where unique_form_id=" & Request.QueryString("uid") & " order by document_id desc"
  	                SQLStmtCM.CommandType = 1
  	                Set SQLStmtCM.ActiveConnection = conn
  	                SQLStmtCM.CommandTimeout = 45 'Timeout per Command
  	               ' response.Write "sql = " & SQLStmtProgramAssignments.CommandText
  	                rsCM.Open SQLStmtCM   

                  document_name = "" 
                  document_name_counter = 0
               
                 Do Until rsCM.EOF

                  if document_name_counter = 0 then
  	                document_name = rsCM("document_name")
                  else
                     document_name = document_name & ", " & rsCM("document_name")
                  end if
  	               
  	                document_name_counter = document_name_counter + 1
                ...
                wfile3.Write( "<value>" & Left(document_name,70) & "</value>" & chr(13) & chr(10)) 

             elseif InStr(singleline, "<!--START ATTACHMENT URL-->") and Request.QueryString("uid") <> "" THEN

             

                 
                Do Until InStr(singleline,"<!--END ATTACHMENT URL-->") <> 0
                    singleline=wfile.readline 
  	            Loop  
                    ...
                    wfile3.write "<!--START ATTACHMENT URL-->" & chr(13) & chr(10)
                   wfile3.write "<url>http://emrhelpdesk.massemr.org/view_attachments.asp?uid=" & Request.QueryString("uid")  & "</url>" & chr(13) & chr(10)
                    wfile3.write "<!--END ATTACHMENT URL-->" & chr(13) & chr(10)
        ...
        elseif InStr(singleline,"<signer_lock_name>") <> 0 and cur_form_status <> "Finalized" THEN
            wfile3.Write("<signer_lock_name>" & cur_login_staff_name & "</signer_lock_name>" & chr(13) & chr(10))  
        

          elseif InStr(singleline,"<submit_form>") <> 0 and cur_form_status <> "Finalized" THEN
               wfile3.Write("<submit_form></submit_form>" & chr(13) & chr(10))  
               

      '  elseif InStr(singleline,"<new_submition>") <> 0 and cur_form_status <> "Finalized" and Request.QueryString("back")="1" THEN
             '   wfile3.Write("<new_submition></new_submition>" & chr(13) & chr(10))  



           elseif InStr(singleline, "<table1>") <> 0 and cur_form_status <> "Finalized" and Request.QueryString("back")<>"1" and Request.QueryString("helpdesk")<>"1" THEN
             ...
             if Request.QueryString("uid") <> "" then
                Set SQLStmt2 = Server.CreateObject("ADODB.Command")
  	            Set rs2 = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmt2.CommandText = "select fxd.xml_data_section.value('(//notify)[1]','varchar(200)') as notify from Staff_Form_XML_Data fxd where unique_form_id=" & Request.QueryString("uid")
  	            SQLStmt2.CommandType = 1
  	            Set SQLStmt2.ActiveConnection = conn
  	            SQLStmt2.CommandTimeout = 45 'Timeout per Command
              ...
              if notify<>"" then
                ...
                wfile3.Write("<row>" & chr(13) & chr(10)) 
                 wfile3.Write("<notes></notes>" & chr(13) & chr(10)) 
                 wfile3.Write("<notify></notify>" & chr(13) & chr(10)) 
                 wfile3.Write("<row_num></row_num>" & chr(13) & chr(10)) 
                 wfile3.Write("<user_stamp>" & Date & " - " & cur_login_staff_name & "</user_stamp>" & chr(13) & chr(10)) 
                 wfile3.Write("<time_stamp></time_stamp>" & chr(13) & chr(10)) 


              if Request.QueryString("uid") <> "" then
                     Set SQLStmtCM = Server.CreateObject("ADODB.Command")
  	                Set rsCM = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtCM.CommandText = "select * from Documents_Master where unique_form_id=" & Request.QueryString("uid") & " order by document_id desc" 
  	                SQLStmtCM.CommandType = 1
  	                Set SQLStmtCM.ActiveConnection = conn
  	                SQLStmtCM.CommandTimeout = 45 'Timeout per Command
  	               ' response.Write "sql = " & SQLStmtProgramAssignments.CommandText
                 ...
                 Do Until rsCM.EOF
  	            ...
  	            Loop
                   ...
                   wfile3.write "<attachments>" & Replace(Replace(document_name,"&","&amp;"),"'","&apos;")  & "</attachments>" & chr(13) & chr(10)
                 else
                 wfile3.Write("<attachments></attachments>" & chr(13) & chr(10)) 
                 end if
            ...
            elseif InStr(singleline,"<attachments>") <> 0 and (cur_form_type = "STAFF_EMR_HELPDESK") and cur_form_status <> "Finalized" THEN
                
              

                    Set SQLStmtCM = Server.CreateObject("ADODB.Command")
  	                Set rsCM = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtCM.CommandText = "select top 1 document_name from Documents_Master where unique_form_id=" & Request.QueryString("uid") & " order by document_id desc"
  	                SQLStmtCM.CommandType = 1
  	                Set SQLStmtCM.ActiveConnection = conn
  	                SQLStmtCM.CommandTimeout = 45 'Timeout per Command
  	               ' response.Write "sql = " & SQLStmtProgramAssignments.CommandText
                 ...
                 Do Until rsCM.EOF
  	            ...
  	            Loop
                   ...
                   wfile3.write "<attachments>" & document_name & "</attachments>" & chr(13) & chr(10)

        elseif InStr(singleline,"<attachments_name>") <> 0 and (cur_form_type = "STAFF_EMR_HELPDESK") and cur_form_status <> "Finalized" THEN
                
              

                    Set SQLStmtCM = Server.CreateObject("ADODB.Command")
  	                Set rsCM = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtCM.CommandText = "select top 1 document_id from Documents_Master where unique_form_id=" & Request.QueryString("uid") & " order by document_id desc"
  	                SQLStmtCM.CommandType = 1
  	                Set SQLStmtCM.ActiveConnection = conn
  	                SQLStmtCM.CommandTimeout = 45 'Timeout per Command
  	               ' response.Write "sql = " & SQLStmtProgramAssignments.CommandText
                 ...
                 Do Until rsCM.EOF
  	                ...
  	                document_id = rsCM("document_id")
                 
  	            
  	            rsCM.MoveNext
  	            Loop

                   wfile3.write "<attachments_name>http://emrhelpdesk.massemr.org/get_doc_file.asp?uid=" & Request.QueryString("uid") & "&amp;did=" & document_id & "</attachments_name>" & chr(13) & chr(10)

        elseif InStr(singleline,"<mandatorycolor>") <> 0 THEN
            wfile3.write(singleline & chr(13) & chr(10) )
            has_mandatory_color_set = 1    
        
         elseif InStr(singleline,"<person_first>") and cur_form_status <> "Finalized" THEN   
            client_first_tag_start = InStr(singleline,"<person_first>")
            client_first_tag_end = InStr(singleline,"</person_first>")
            total_tag_length = client_first_tag_end - client_first_tag_start
            old_form_first = Mid(singleline,(client_first_tag_start+14),(total_tag_length-14))                                      
            wfile3.Write("<person_first>"& rsC("First_Name") & "</person_first>" & chr(13) & chr(10))   
            
        elseif InStr(singleline,"<person_middle>") and cur_form_status <> "Finalized" THEN       
            client_middle_tag_start = InStr(singleline,"<person_middle>")
            client_middle_tag_end = InStr(singleline,"</person_middle>")
            total_tag_length = client_middle_tag_end - client_middle_tag_start
            old_form_middle = Mid(singleline,(client_middle_tag_start+15),(total_tag_length-15))
            wfile3.Write(Replace(singleline, "<person_middle>"& old_form_middle & "</person_middle>", "<person_middle>"& rsC("Middle_Name") & "</person_middle>") & chr(13) & chr(10))   
        elseif InStr(singleline,"<person_last>") and cur_form_status <> "Finalized" THEN
            client_last_tag_start = InStr(singleline,"<person_last>")
            client_last_tag_end = InStr(singleline,"</person_last>")
            total_tag_length = client_last_tag_end - client_last_tag_start
            old_form_last = Mid(singleline,(client_last_tag_start+13),(total_tag_length-13))
            wfile3.Write(Replace(singleline, "<person_last>"& old_form_last & "</person_last>", "<person_last>"& rsC("Last_Name") & "</person_last>") & chr(13) & chr(10))   
       ...
       elseif InStr(singleline,"<is_edit") <> 0 and (Request.QueryString("uid") <> "" and Request.QueryString("back")<>"1" and Request.QueryString("helpdesk")<>"1" and cur_form_status <> "Finalized") THEN
                wfile3.Write("          <is_edit>1</is_edit>") & chr(13) & chr(10)
     ...
     ' elseif InStr(singleline,"<table1>") <> 0 THEN 

               '   Do Until InStr(singleline,"</table1>") <> 0 or wfile.AtEndOfStream  
               
                   
               
                 '  repeat_table =  repeat_table & singleline & chr(13) & chr(10)
                  ' singleline=wfile.readline
                   
                   
              ' Loop

              ' wfile3.Write(repeat_table & chr(13) & chr(10))
            ' wfile3.Write("<row>" & chr(13) & chr(10))
            
            ' wfile3.Write("</row>" & chr(13) & chr(10))
            '   wfile3.Write("</table1>" & chr(13) & chr(10))

        elseif InStr(singleline,"<new_submition>") and cur_form_status <> "Finalized" THEN
               wfile3.Write( "<new_submition>1</new_submition>" & chr(13) & chr(10))   
        elseif InStr(singleline,"<new_submition>") and cur_form_status = "Finalized" THEN
               wfile3.Write( "<new_submition>0</new_submition>" & chr(13) & chr(10))     
        elseif InStr(singleline,"<logged_in_user>") and cur_form_status <> "Finalized" THEN
               wfile3.Write( "<logged_in_user>" & Date & " - " & cur_login_staff_name  & "</logged_in_user>" & chr(13) & chr(10))       
     ...
     '   elseif InStr(singleline,"<data sid=""form_logo1"">") <> 0 THEN   
       ...
       elseif (InStr(singleline,"<checkgroup sid=""Notify"">") or InStr(singleline,"<checkgroup sid=""Notify1"">")) and agency_program_id =-1 THEN
                    ...
                    'IN THE DYNAMIC SECTION WRITE THIS LINE THEN START LOOPS
                    Do Until InStr(singleline,"</xforms:select>")
                        'wfile3.write(singleline & chr(13) & chr(10))
                        singleline=wfile.readline 
                    loop
                
                    cur_tracking_options_count = 1
                    tracking_row_num = 0                
                



                         if cur_tracking_options_count = 1 THEN
                            x_coord = "1"
                            y_coord = 2 + (tracking_row_num*22)
                        elseif cur_tracking_options_count = 2 THEN
                            x_coord = "190"
                            y_coord = 2 + (tracking_row_num*22)
                        elseif cur_tracking_options_count = 3 THEN
                            x_coord = "390"
                            y_coord = 2 + (tracking_row_num*22)
                       elseif cur_tracking_options_count = 4 THEN
                            x_coord = "590"
                            y_coord = 2 + (tracking_row_num*22)
                            tracking_row_num = tracking_row_num + 1
                            cur_tracking_options_count = 0  
                        end if    
                        ...
                        wfile3.Write ("   <xforms:label>EMR HELPDESK</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>1344</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                      ...
                      cur_tracking_options_count = cur_tracking_options_count + 1
                   ...
                   if  agency_program_id =-1 then
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>GLOBAL BROADCAST</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>-2</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                   ...
                   if agency_program_id =-1 or agency_contact=1 then  

                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>Julie Suyer</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>1335</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                      ...
                      if cur_tracking_options_count = 1 THEN
                       ...
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>Eric Larochelle</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>1336</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                      ...
                      if cur_tracking_options_count = 1 THEN
                       ...
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>AGENCY CONTACT(S)</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>-4</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                    ...
                    if agency_program_id =-1 or agency_contact=1 then  
                       ...
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>C4C</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>-3</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                   ...
                   if agency_program_id =-1 or agency_contact=1 then 
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>Alex Suyer</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>9</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                     ...
                     cur_tracking_options_count = cur_tracking_options_count + 1
                      ...
                      if cur_tracking_options_count = 1 THEN
                       ...
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>Peter Card</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>7</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                     ...
                     cur_tracking_options_count = cur_tracking_options_count + 1
                      ...
                      if cur_tracking_options_count = 1 THEN
                       ...
                       elseif cur_tracking_options_count = 4 THEN
                        ...
                        wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>NA</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>-10</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
                    ...
                    Set SQLStmtV = Server.CreateObject("ADODB.Command")
  	                Set rsV = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtV.CommandText = "select Staff_ID,Last_Name,First_Name from staff_master where emr_staff=1 and Staff_ID NOT IN(7,9,1335,1336,1344) order by emr_staff" 
  	                SQLStmtV.CommandType = 1
  	                Set SQLStmtV.ActiveConnection = conn
  	                SQLStmtV.CommandTimeout = 45 'Timeout per Command
  	                rsV.Open SQLStmtV
  	                ...
  	                Do Until rsV.EOF  	          
                       ...
                       elseif cur_tracking_options_count = 4 THEN
                        ...
                        wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>" & rsV("First_Name") & " "  & rsV("Last_Name") & "</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>"  & rsV("Staff_ID") & "</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>" & x_coord & "</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>" & y_coord & "</y>" & chr(13) & chr(10))
      ...
      elseif (InStr(singleline,"<checkgroup sid=""Case_Note_Type"">") or InStr(singleline,"<checkgroup sid=""Case_Note_Type"">")) and agency_program_id =-1 THEN
                    ...
                    'IN THE DYNAMIC SECTION WRITE THIS LINE THEN START LOOPS
                    Do Until InStr(singleline,"</checkgroup>")
                        'wfile3.write(singleline & chr(13) & chr(10))
                        singleline=wfile.readline 
                    loop
            ...
            wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>OPEN</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>OPEN</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <x>2</x>" & chr(13) & chr(10))
                        wfile3.Write ("         <y>6</y>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("      <labelfontinfo>" & chr(13) & chr(10))
                        wfile3.Write ("         <fontname>Arial</fontname>" & chr(13) & chr(10))
                        wfile3.Write ("          <size>8</size>" & chr(13) & chr(10))
                     ...
                     wfile3.write("<!--START EXCLUDE DD-->" & chr(13) & chr(10))
                      wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>CLOSED</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>CLOSED</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <after compute=""itemprevious""></after>" & chr(13) & chr(10))
                        wfile3.Write ("         <offsetx>52</offsetx>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("      <labelfontinfo>" & chr(13) & chr(10))
                        wfile3.Write ("         <fontname>Arial</fontname>" & chr(13) & chr(10))
                        wfile3.Write ("          <size>8</size>" & chr(13) & chr(10))
                        ...
                        wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>ASSIGNED</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>ASSIGNED</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <after compute=""itemprevious""></after>" & chr(13) & chr(10))
                        wfile3.Write ("         <offsetx>52</offsetx>" & chr(13) & chr(10))
                     ...
                     wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>HOLD</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>HOLD</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <after compute=""itemprevious""></after>" & chr(13) & chr(10))
                        wfile3.Write ("         <offsetx>64</offsetx>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("      <labelfontinfo>" & chr(13) & chr(10))
                        wfile3.Write ("         <fontname>Arial</fontname>" & chr(13) & chr(10))
                        wfile3.Write ("          <size>8</size>" & chr(13) & chr(10))
                       ...
                       wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>FUTURE</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>FUTURE</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <after compute=""itemprevious""></after>" & chr(13) & chr(10))
                        wfile3.Write ("         <offsetx>80</offsetx>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("      <labelfontinfo>" & chr(13) & chr(10))
                        wfile3.Write ("         <fontname>Arial</fontname>" & chr(13) & chr(10))
                        wfile3.Write ("          <size>8</size>" & chr(13) & chr(10))
                        ...
                        wfile3.Write ("<xforms:item>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:label>NEXT</xforms:label>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:value>NEXT</xforms:value>" & chr(13) & chr(10))
                        wfile3.Write ("   <xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("       <itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("         <after compute=""itemprevious""></after>" & chr(13) & chr(10))
                        wfile3.Write ("         <offsetx>82</offsetx>" & chr(13) & chr(10))
                        wfile3.Write ("      </itemlocation>" & chr(13) & chr(10))
                        wfile3.Write ("      <labelfontinfo>" & chr(13) & chr(10))
                        wfile3.Write ("         <fontname>Arial</fontname>" & chr(13) & chr(10))
                        wfile3.Write ("          <size>8</size>" & chr(13) & chr(10))
                        ...
                        wfile3.Write ("   </xforms:extension>" & chr(13) & chr(10))
                        wfile3.Write ("</xforms:item>" & chr(13) & chr(10))
                     wfile3.write("<!--END EXCLUDE DD-->" & chr(13) & chr(10))
                     wfile3.write("</xforms:select1>" & chr(13) & chr(10))
                     wfile3.write("<itemlocation>" & chr(13) & chr(10))
                     wfile3.write("<after compute=""itemprevious""></after>" & chr(13) & chr(10))
                     wfile3.write("<width>762</width>" & chr(13) & chr(10))
                     wfile3.write("<height>22</height>" & chr(13) & chr(10))
                     wfile3.write("<offsetx>-8</offsetx>" & chr(13) & chr(10))
                     wfile3.write("<offsety>-3</offsety>" & chr(13) & chr(10))
                     wfile3.write("</itemlocation>" & chr(13) & chr(10))
                     wfile3.write("<format>" & chr(13) & chr(10))
                     wfile3.write("<datatype>string</datatype>" & chr(13) & chr(10))
                     wfile3.write("<constraints>" & chr(13) & chr(10))
                     wfile3.write("<mandatory>on</mandatory>" & chr(13) & chr(10))
                      wfile3.write("</constraints>" & chr(13) & chr(10))
                     wfile3.write("</format>" & chr(13) & chr(10))
                     wfile3.write("<readonly>off</readonly>" & chr(13) & chr(10))                  
...
<%               
  	    ...
  	    if Request.QueryString("ft") <> "" THEN  	    
            ...
            'GET STAFF INFO
            Set SQLStmtStaff = Server.CreateObject("ADODB.Command")
            Set rsStaff = Server.CreateObject ("ADODB.Recordset")
  	        SQLStmtStaff.CommandText = "get_staff_info " & Request.QueryString("sid")
  	        SQLStmtStaff.CommandType = 1
  	        Set SQLStmtStaff.ActiveConnection = conn
  	        SQLStmtStaff.CommandTimeout = 45 'Timeout per Command
 	        rsStaff.Open SQLStmtStaff
 	        
 	        cur_staff_id = rsStaff("Staff_ID")
 	        cur_staff_name = rsStaff("Staff_Name")
 	        cur_first_name = rsStaff("First_Name")
 	        cur_middle_name = rsStaff("Middle_Name")
 	        cur_last_name = rsStaff("Last_Name")
			cur_staff_address = rsStaff("staff_address")
			cur_staff_city = rsStaff("staff_city")
			cur_staff_state = rsStaff("staff_state")
			cur_staff_zip = rsStaff("staff_zip")
			cur_staff_home_phone = rsStaff("home_phone")
			cur_staff_work_phone = rsStaff("work_phone")
			cur_staff_cell_phone = rsStaff("cell_phone")
			cur_staff_email_address = rsStaff("email_address")
            ...
            elseif InStr(singleline,"<staff_name></staff_name>") and foundClientName = 0 THEN
               wfile3.Write("<staff_name>" & cur_staff_name & "</staff_name>" & chr(13) & chr(10)) 
            elseif InStr(singleline,"<trainee></trainee>") and foundClientName = 0 THEN
               wfile3.Write("<trainee>" & cur_staff_name & "</trainee>" & chr(13) & chr(10))   			   
             elseif InStr(singleline,"<form_staff_id></form_staff_id>") and foundClientName = 0 and cur_form_status <> "Finalized"  THEN
               wfile3.Write("<form_staff_id>" & cur_staff_id & "</form_staff_id>" & chr(13) & chr(10))                
            elseif InStr(singleline,"<person_first></person_first>") and foundClientName = 0 THEN
               wfile3.Write("<person_first>" & cur_first_name & "</person_first>" & chr(13) & chr(10)) 
            elseif InStr(singleline,"<person_middle></person_middle>") and foundClientName = 0 THEN
               wfile3.Write("<person_middle>" & cur_middle_name & "</person_middle>" & chr(13) & chr(10)) 
            elseif InStr(singleline,"<person_last></person_last>") and foundClientName = 0 THEN
               wfile3.Write("<person_last>" & cur_last_name & "</person_last>" & chr(13) & chr(10))     
            
          elseif InStr(singleline,"<md_first></md_first") and foundClientName = 0 THEN
               wfile3.Write("<md_first>" & cur_first_name & "</md_first>" & chr(13) & chr(10)) 
            
            elseif InStr(singleline,"<md_last></md_last>") and foundClientName = 0 THEN
               wfile3.Write("<md_last>" & cur_last_name & "</md_last>" & chr(13) & chr(10))     

         elseif InStr(singleline,"<provider_street></provider_street>") and foundClientName = 0 THEN
               wfile3.Write("<provider_street>" & cur_staff_address & "</provider_street>" & chr(13) & chr(10)) 
         elseif InStr(singleline,"<provider_city></provider_city>") and foundClientName = 0 THEN
               wfile3.Write("<provider_city>" & cur_staff_city & "</provider_city>" & chr(13) & chr(10))    
          elseif InStr(singleline,"<contact_state></contact_state>") and foundClientName = 0 THEN
               wfile3.Write("<contact_state>" & cur_staff_state & "</contact_state>" & chr(13) & chr(10))     
          elseif InStr(singleline,"<provider_zip></provider_zip>") and foundClientName = 0 THEN
               wfile3.Write("<provider_zip>" & cur_staff_zip & "</provider_zip>" & chr(13) & chr(10))    
          elseif InStr(singleline,"<home_phone></home_phone>") and foundClientName = 0 THEN
               wfile3.Write("<home_phone>" & cur_staff_home_phone & "</home_phone>" & chr(13) & chr(10)) 
          elseif InStr(singleline,"<work_phone></work_phone>") and foundClientName = 0 THEN
               wfile3.Write("<work_phone>" & cur_staff_work_phone & "</work_phone>" & chr(13) & chr(10)) 
          elseif InStr(singleline,"<cell_phone></cell_phone>") and foundClientName = 0 THEN
               wfile3.Write("<cell_phone>" & cur_staff_cell_phone & "</cell_phone>" & chr(13) & chr(10)) 
          elseif InStr(singleline,"<email></email>") and foundClientName = 0 THEN
               wfile3.Write("<email>" & cur_staff_email_address  & "</email>" & chr(13) & chr(10)) 			   
          ...
          elseif InStr(singleline,"<form_id></form_id>") and foundFormID = 0 THEN
           ...
           elseif InStr(singleline,"<form_staff_id></form_staff_id>") THEN
               wfile3.Write("<form_staff_id>" & Request.QueryString("sid") & "</form_staff_id>" & chr(13) & chr(10)) 
          ...
          '  elseif InStr(singleline,"<gender></gender>") <> 0 then
           ...
           ' elseif InStr(singleline,"<admit_date></admit_date>") <> 0 then
            ...
            elseif InStr(singleline, "<table2>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
             
                wfile3.Write(singleline & chr(13) & chr(10) )
                
                Do Until InStr(singleline, "</table2>") <> 0
  	                    singleline=wfile.readline 
  	            Loop   	            
                


               IF Request.QueryString("lfid") <> "" THEN
                ...
                Set SQLStmtPCTD = Server.CreateObject("ADODB.Command")
  	            Set rsPCTD = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtPCTD.CommandText = "exec get_previous_change_tracking_dates " & Request.QueryString("lfid")
  	            SQLStmtPCTD.CommandType = 1
  	            Set SQLStmtPCTD.ActiveConnection = conn
  	            SQLStmtPCTD.CommandTimeout = 45 'Timeout per Command

  	          '  wfile3.Write("          <!--" & SQLStmtBenefits.CommandText & "-->") & chr(13) & chr(10)

  	            rsPCTD.Open SQLStmtPCTD
  	            ...
  	            Do Until rsPCTD.EOF 
                           ...
                           open_date = rsPCTD("open_date")
                          closed_date = rsPCTD("closed_date")
                       subject = rsPCTD("subject")
                      status = rsPCTD("status")
               ...
               Loop
  	            ...
  	            Set SQLStmtBenefits = Server.CreateObject("ADODB.Command")
  	            Set rsBenefits = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtBenefits.CommandText = "exec get_previous_change_tracking_form " & Request.QueryString("lfid")
  	            SQLStmtBenefits.CommandType = 1
  	            Set SQLStmtBenefits.ActiveConnection = conn
  	            SQLStmtBenefits.CommandTimeout = 45 'Timeout per Command

  	          '  wfile3.Write("          <!--" & SQLStmtBenefits.CommandText & "-->") & chr(13) & chr(10)

  	            rsBenefits.Open SQLStmtBenefits
  	            
  	            num_benefits = 0

              

  	            Do Until rsBenefits.EOF 
  	                
  	                wfile3.Write("<row>") & chr(13) & chr(10)
                    wfile3.Write("          <notes1>" & REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(rsBenefits("notes"),"&","&amp;"),"'","&apos;"),"""","&quot;"),"<","&lt;"),">","&gt;") & "</notes1>") & chr(13) & chr(10)
                   
                     wfile3.Write("          <notify1>" & rsBenefits("notify") & "</notify1>") & chr(13) & chr(10)
                          wfile3.Write("          <user_stamp1>" & rsBenefits("user_stamp") & "</user_stamp1>") & chr(13) & chr(10)
                        wfile3.Write("          <time_stamp>" & rsBenefits("user_stamp") & "</time_stamp>") & chr(13) & chr(10)
  	            ...
  	            'write out end of table now
  	            wfile3.Write(singleline & chr(13) & chr(10) )

          elseif InStr(singleline, "<program_id></program_id>") <> 0  THEN 
                wfile3.Write "<program_id>" & agency_program_id & "</program_id>" & chr(13) & chr(10)

           elseif InStr(singleline, "<table2>") <> 0 and (Request.QueryString("ft") = "STAFF_TRACK_REPORT") THEN
               ...
               close_date = ""
               if ISDATE(Request.QueryString("cd")) then
                 close_date = CStr(FormatDateTime(Request.QueryString("cd"),2))
               end if

              cur_notify=Request.QueryString("notify")
           


                   

                           if IsNumeric(cur_notify) and cur_notify <> "-4" then

                                   Set SQLStmtQNR3 = Server.CreateObject("ADODB.Command")
                                   Set rsQNR3 = Server.CreateObject ("ADODB.Recordset")                
                                 
                                   SQLStmtQNR3.CommandText = "select * from staff_master where agency_contact=1 and staff_id=" &  Request.QueryString("notify") 
                                  
                                    SQLStmtQNR3.CommandType = 1 
                                    Set SQLStmtQNR3.ActiveConnection = conn
                                    SQLStmtQNR3.CommandTimeout = 45 'Timeout per Command
                                  
                                    rsQNR3.Open SQLStmtQNR3
                           
                                   wfile3.Write("<!--" &   SQLStmtQNR3.CommandText & "-->") & chr(13) & chr(10)
                ...
                cur_agency=Request.QueryString("agency")




                               if instr(cur_notify,"-4")>0 and cur_agency="-1" then
           

                                   Set SQLStmtQNR3 = Server.CreateObject("ADODB.Command")
                                   Set rsQNR3 = Server.CreateObject ("ADODB.Recordset")                
                                 
                                   SQLStmtQNR3.CommandText = "select Program_ID,Program_Name from Program_Master where CHARINDEX('-',Program_Name)=0 and PROGRAM_NAME<>'INTAKE' and PROGRAM_NAME<>'ALL AGENCIES' and PROGRAM_NAME<>'DD EMR' and program_id in(select main_program from staff_forms_master where form_type='STAFF_EMR_HELPDESK') and program_id in(select Program_ID from Staff_Program_Assign where staff_id=" & Request.QueryString("notify") & ") order by Program_Name asc" 
  	            ...
  	            Set SQLStmtBenefits = Server.CreateObject("ADODB.Command")
  	            Set rsBenefits = Server.CreateObject("ADODB.Recordset")
  	            SQLStmtBenefits.CommandText = "exec get_previous_change_tracking_detail_report '" & Replace(Replace(Request.QueryString("search"),"^"," "),"'","''") & "','" &  Request.QueryString("od") & "','" &  Request.QueryString("cd") & "','" &  Request.QueryString("status") & "','" & Request.QueryString("submission_type") & "'," & cur_agency & ",'" & cur_notify & "','" & Request.QueryString("aging") & "'," & cur_staff_id & "," & agency_program_id & "," & Request.QueryString("atm")  & "," & Request.QueryString("ic") & ",'" & session("user_name") & "'"
  	            SQLStmtBenefits.CommandType = 1
  	            Set SQLStmtBenefits.ActiveConnection = conn
  	            SQLStmtBenefits.CommandTimeout = 45 'Timeout per Command
     'response.write SQLStmtBenefits.CommandText
  	            wfile3.Write("<!--" & SQLStmtBenefits.CommandText & "-->") & chr(13) & chr(10)
                  ...
                  subject = rsBenefits("subject")
                  parent_unique_form_id = rsBenefits("unique_form_id")
                 ...
                 Set SQLStmtPCTDR2 = Server.CreateObject("ADODB.Command")
  	            Set rsPCTDR2 = Server.CreateObject("ADODB.Recordset")
  	        '    SQLStmtPCTDR2.CommandText = "exec get_previous_change_tracking_status '" & subject & "'"

    SQLStmtPCTDR2.CommandText = "exec get_previous_change_tracking_status " & parent_unique_form_id & ",'" & cur_notify & "'"
  	            SQLStmtPCTDR2.CommandType = 1
  	            Set SQLStmtPCTDR2.ActiveConnection = conn
  	            SQLStmtPCTDR2.CommandTimeout = 45 'Timeout per Command
   ' response.write SQLStmtPCTDR2.CommandText
             wfile3.Write("          <!--" & SQLStmtPCTDR2.CommandText & "-->") & chr(13) & chr(10)

                 rsPCTDR2.Open SQLStmtPCTDR2

                     
                       

                    Do Until rsPCTDR2.EOF 


                      open_date = rsPCTDR2("open_date")
                       closed_date = rsPCTDR2("closed_date")
                       submission_type = rsPCTDR2("submission_type")
                       cur_unique_form_id = rsPCTDR2("unique_form_id")
                       agency = rsPCTDR2("agency")

                   ' wfile3.Write("          <case_note>" & REPLACE(rsBenefits("notes"),"&","&amp;") & "</case_note>") & chr(13) & chr(10)
                    
                    wfile3.Write("          <open_date>" & open_date & "</open_date>") & chr(13) & chr(10)
                    wfile3.Write("          <close_date>" & closed_date & "</close_date>") & chr(13) & chr(10)
                     wfile3.Write("          <submission_type>" & Replace(submission_type,"_"," ") & "</submission_type>") & chr(13) & chr(10)
                        wfile3.Write("          <agency>" & UCASE(agency) & "</agency>") & chr(13) & chr(10)
                    if closed_date = "" then
                       wfile3.Write("          <note_status>OPEN</note_status>") & chr(13) & chr(10)
                    else
                       wfile3.Write("          <note_status>CLOSED</note_status>") & chr(13) & chr(10)
                    end if
                 ...
                 Set SQLStmtPCTDR = Server.CreateObject("ADODB.Command")
  	            Set rsPCTDR = Server.CreateObject("ADODB.Recordset")
  	            SQLStmtPCTDR.CommandText = "exec get_previous_change_tracking_detail_report_notes " & cur_unique_form_id & ",'" & cur_notify & "'" 
  	            SQLStmtPCTDR.CommandType = 1
  	            Set SQLStmtPCTDR.ActiveConnection = conn
  	            SQLStmtPCTDR.CommandTimeout = 45 'Timeout per Command
   
  	          wfile3.Write("          <!--" & SQLStmtPCTDR.CommandText & "-->") & chr(13) & chr(10)

  	            rsPCTDR.Open SQLStmtPCTDR
  	            
  	
            entry_count=0                 

            wfile3.Write("<entries>") & chr(13) & chr(10)

  	            Do Until rsPCTDR.EOF 
                      ...
                      entry_count=entry_count+1    
                            ...
                            notes=rsPCTDR("notes")
                            user_stamp=rsPCTDR("user_stamp")
                            notify=rsPCTDR("notify")
                            agency=rsPCTDR("agency")
                           
              
                        wfile3.Write("<entry>") & chr(13) & chr(10)
                        wfile3.Write("          <row_num>" & entry_count & "</row_num>") & chr(13) & chr(10)
                        wfile3.Write("          <subject>" & REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UCASE(subject),"&","&amp;"),"'","&apos;"),"""","&quot;"),"<","&lt;"),">","&gt;"),"""","&quot;")    & "</subject>") & chr(13) & chr(10)
                        wfile3.Write("          <notes>" & REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(notes,"&","&amp;"),"'","&apos;"),"""","&quot;"),"<","&lt;"),">","&gt;"),"""","&quot;")  & "</notes>") & chr(13) & chr(10)
                      if entry_count=1 then
                        wfile3.Write("          <user_stamp>" & UCASE(user_stamp) & "</user_stamp>") & chr(13) & chr(10)
                      else
                         wfile3.Write("          <user_stamp>"  & UCASE(user_stamp) & "</user_stamp>") & chr(13) & chr(10)
                      end if
                        wfile3.Write("          <url>http://" & url_org_name & "/view_attachments.asp?uid=" & cur_unique_form_id & "&amp;fromStaff=1</url>") & chr(13) & chr(10)
                        wfile3.Write("<notes>" & REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(notes,"&","&amp;"),"'","&apos;"),"""","&quot;"),"<","&lt;"),">","&gt;"),"""","&quot;")  & "</notes>") & chr(13) & chr(10)

                               if rsPCTDR("notify") <> "" then

                                Set SQLStmtPCTDR3 = Server.CreateObject("ADODB.Command")
  	                            Set rsPCTDR3 = Server.CreateObject("ADODB.Recordset")
  	                            SQLStmtPCTDR3.CommandText = "exec get_staff_from_tracking " & cur_unique_form_id & ",'" & replace(notify," ",",") & "'"
  	                            SQLStmtPCTDR3.CommandType = 1
  	                            Set SQLStmtPCTDR3.ActiveConnection = conn
  	                            SQLStmtPCTDR3.CommandTimeout = 45 'Timeout per Command
                              ' response.write SQLStmtPCTDR3.CommandText
  	                           wfile3.Write("          <!--" & SQLStmtPCTDR3.CommandText & "-->") & chr(13) & chr(10)

  	                            rsPCTDR3.Open SQLStmtPCTDR3
                         notify = ""

                          notify_count = 0

                                Do Until rsPCTDR3.EOF 
                                    
                                  if notify_count = 0 then
                                    notify = notify & rsPCTDR3("user_name") 
                                  else
                                     notify = notify &  ", " & rsPCTDR3("user_name")  
                                 end if
                         ...
                         wfile3.Write("          <staff_notified>" & notify & "</staff_notified>") & chr(13) & chr(10)
                        ...
                        Set SQLStmtCM = Server.CreateObject("ADODB.Command")
  	                Set rsCM = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtCM.CommandText = "select document_name from Documents_Master where unique_form_id=" & cur_unique_form_id
  	                SQLStmtCM.CommandType = 1
  	                Set SQLStmtCM.ActiveConnection = conn
  	                SQLStmtCM.CommandTimeout = 45 'Timeout per Command
  	               ' response.Write "sql = " & SQLStmtProgramAssignments.CommandText
                 ...
                 Do Until rsCM.EOF
  	            ...
  	            Loop
                   ...
                   wfile3.write "<attachments>" & Replace(Replace(Mid(document_name,1,50),"&","&amp;"),"'","&apos;") & "</attachments>" & chr(13) & chr(10)
                 ...
                 rsPCTDR.MoveNext  	            
  	            ...
  	            Loop
                       ...
                       wfile3.Write("<entry>") & chr(13) & chr(10)
                        wfile3.Write("          <notes></notes>") & chr(13) & chr(10)
                             wfile3.Write("          <user_stamp></user_stamp>") & chr(13) & chr(10)
                         wfile3.Write("          <staff_notified></staff_notified>") & chr(13) & chr(10)
                       wfile3.Write("</entry>") & chr(13) & chr(10)
         ...
         elseif InStr(singleline,"<form_count>") <> 0 and (Request.QueryString("ft") = "STAFF_TRACK_REPORT") THEN
                wfile3.Write("          <form_count>" & num_benefits & "</form_count>") & chr(13) & chr(10)
                  
         elseif InStr(singleline,"<agency>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK" and agency_program_id <> -1) THEN
                wfile3.Write("          <agency>" & agency_program_id & "</agency>") & chr(13) & chr(10)

        elseif InStr(singleline,"<open_date></open_date>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
                wfile3.Write("          <open_date>" & open_date & "</open_date>") & chr(13) & chr(10)

            elseif InStr(singleline,"<closed_date></closed_date>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
                wfile3.Write("          <closed_date>" & closed_date & "</closed_date>") & chr(13) & chr(10)

         
            elseif InStr(singleline,"<case_note_type></case_note_type>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
                 wfile3.Write("          <case_note_type>" & status & "</case_note_type>") & chr(13) & chr(10)
            elseif InStr(singleline,"<subject></subject>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
                 wfile3.Write("          <subject>" & subject & "</subject>") & chr(13) & chr(10)
            elseif InStr(singleline,"<subject_read_only></subject_read_only>") <> 0 and (Request.QueryString("ft") = "STAFF_EMR_HELPDESK") THEN
                if subject <> "" then
                 wfile3.Write("          <subject_read_only>1</subject_read_only>") & chr(13) & chr(10)
               else
                 wfile3.Write("          <subject_read_only></subject_read_only>") & chr(13) & chr(10)
               end if
  	    ...
  	    elseif InStr(singleline,"<other_meds_plain_text></other_meds_plain_text>") <> 0 THEN
           ...
           elseif InStr(singleline,"<user_stamp>") THEN
               wfile3.Write( "<user_stamp>" & Date & " - " & cur_login_staff_name  & "</user_stamp>" & chr(13) & chr(10))    
            elseif InStr(singleline,"<time_stamp>") THEN
               wfile3.Write( "<time_stamp>" & Now() & "</time_stamp>" & chr(13) & chr(10))    
          ...
          elseif InStr(singleline,"<logged_in_user>") THEN
               wfile3.Write( "<logged_in_user>" & Date & " - " & cur_login_staff_name  & "</logged_in_user>" & chr(13) & chr(10))  
        ...
        '***********************************************************************************
            ...
            elseif InStr(singleline,"<xforms:instance id=""agency"" xmlns="""">") THEN
                wfile3.Write (singleline & chr(13) & chr(10))
                singleline=wfile.readline 
                wfile3.Write (singleline & chr(13) & chr(10))
                ...
                Set SQLStmtV = Server.CreateObject("ADODB.Command")
  	            Set rsV = Server.CreateObject ("ADODB.Recordset")
               if agency_program_id = -1 then
  	            SQLStmtV.CommandText = "select Program_ID,Program_Name from Program_Master where Is_Active=1 and CHARINDEX('-',Program_Name)=0 and Program_Name<>'Intake' order by Program_Name"
               else
                SQLStmtV.CommandText = "select Program_ID,Program_Name from Program_Master where Program_ID=" & agency_program_id
  	            ...
  	            Do Until rsV.EOF
                     ...
                     wfile3.Write ("<choice value=""" & rsV("Program_ID") & """>" & rsv("Program_Name") & "</choice>" & chr(13) & chr(10))
                     '<choice value="319">ABC-XYZ</choice>
          ...
          elseif InStr(singleline,"<data>") <> 0 THEN
                wfile3.write(singleline & chr(13) & chr(10))
                found_data_start = 1
                
                if Request.QueryString("ft") = "STAFF_INTAKE" THEN
                    Set SQLStmtX = Server.CreateObject("ADODB.Command")
  	                Set rsX = Server.CreateObject ("ADODB.Recordset")
                    SQLStmtX.CommandText = "select top 1 unique_form_id from staff_forms_master where staff_id = " & Request.QueryString("sid") & " and form_type = 'STAFF_INTAKE' order by update_date desc"
  	                SQLStmtX.CommandType = 1
  	                Set SQLStmtX.ActiveConnection = conn
  	                SQLStmtX.CommandTimeout = 45 'Timeout per Command
  	              '  response.Write "sql = " & SQLStmtX.CommandText
  	                rsX.Open SQLStmtX
      	            
  	                if rsX.EOF THEN
  	                    intake_to_prepop_from = -1
  	                else
  	                    intake_to_prepop_from = rsX("unique_form_id")
  	                end if
			 ...
			 elseif InStr(singleline,"<data>") <> 0 THEN
                wfile3.write(singleline & chr(13) & chr(10))
                found_data_start = 1	
			  elseif Request.QueryString("ft") = "STAFF_INTAKE" and found_data_start = 1 and found_data_end <> 1 and InStr(singleline,"<") <> 0 and InStr(singleline,">") <> 0 and intake_to_prepop_from <> -1 and InStr(singleline,"<choices") = 0 and InStr(singleline,"<emr_allergies>") = 0 and InStr(singleline,"<form_id>") = 0 and InStr(singleline,"<signer_lock_name>") = 0 and InStr(singleline,"<gender>") = 0 and InStr(singleline,"intake_title") = 0 and InStr(singleline,"ssn") = 0 and InStr(singleline,"md_first") = 0 and InStr(singleline,"md_last") = 0 and InStr(singleline,"md_address") = 0 and InStr(singleline,"md_city") = 0 and InStr(singleline,"contact_state_2") = 0 and InStr(singleline,"md_zip") = 0 and InStr(singleline,"md_phone") = 0 and InStr(singleline,"md_fax") = 0 and InStr(singleline,"pcp_email") = 0 and InStr(singleline,"md_description") = 0 THEN 
                                  
                cur_tag_start = InStr(singleline,"<")                
                cur_tag_end = InStr(singleline,">")                
                total_tag_length = cur_tag_end - cur_tag_start                
                cur_tag_name = Mid(singleline, cur_tag_start+1, total_tag_length-1)
                                
                if InStr(cur_tag_name, "/") = 0 and InStr(singleline, "</") <> 0 and InStr(singleline, "choice value") = 0 THEN
                    Set SQLStmtX = Server.CreateObject("ADODB.Command")
  	                Set rsX = Server.CreateObject ("ADODB.Recordset")
  	                SQLStmtX.CommandText = "select case when fxd.xml_data_section.value('(//" & cur_tag_name & ")[1]','varchar(5000)') is null then 'SKIPTHIS' else fxd.xml_data_section.value('(//" & cur_tag_name & ")[1]','varchar(5000)') end as old_tag_value from Staff_Form_XML_Data fxd where fxd.unique_form_id = " & intake_to_prepop_from
  	             ...
  	             '   response.Write "sql = " & SQLStmtX.CommandText
  	                ...
  	                old_tag_value = rsX("old_tag_value")
          	            
  	                if old_tag_value <> "" and old_tag_value <> "SKIPTHIS" and cur_tag_name <> "form_id" THEN
  	                    wfile3.write("<" & cur_tag_name & ">" & REPLACE(REPLACE(old_tag_value,"&","&amp;"),"<", "&lt;") & "</" & cur_tag_name & ">" & chr(13) & chr(10))
  	                else
  	                    wfile3.write(singleline & chr(13) & chr(10))
  	                end if
  	            else
          ...
          elseif InStr(singleline,"<xforms:instance id=""Submission"" xmlns="""">") THEN
                wfile3.Write (singleline & chr(13) & chr(10))
                singleline=wfile.readline 
                wfile3.Write (singleline & chr(13) & chr(10))
                ...
                Set SQLStmtV = Server.CreateObject("ADODB.Command")
  	            Set rsV = Server.CreateObject ("ADODB.Recordset")
  	            SQLStmtV.CommandText = "select code_name,long_desc from code_map where code_type=(select top 1 code_type from Code_Def where form_sid='Submission_Type')"
  	            SQLStmtV.CommandType = 1
  	            Set SQLStmtV.ActiveConnection = conn
  	            SQLStmtV.CommandTimeout = 45 'Timeout per Command
  	            rsV.Open SQLStmtV
  	            ...
  	            Do Until rsV.EOF
                     ...
                     wfile3.Write ("<choice value=""" & rsV("code_name") & """>" & rsv("long_desc") & "</choice>" & chr(13) & chr(10))
        ...
        do while not wfile.AtEndOfStream and model_end_found = 1
            ...
            singleline=wfile.readline
             ...
             if InStr(singleline,"<mimedata encoding=") <> 0 and  Request.QueryString("ft") = "STAFF_INTAKE" THEN ' EFS and INTAKE need a picture
...
" from staff_master as qs where staff_id = " &  Request.QueryString("sid")
       ...
       elseif InStr(singleline,"<!--START EXCLUDE DD-->") <> 0 and agency_program_id <>-1 THEN
                ...
                wfile3.Write("test" & chr(13) & chr(10)) 
